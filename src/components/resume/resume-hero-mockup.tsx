import { CheckSquare, ShieldCheck, Briefcase, Sparkles, Mail, Phone, MapPin } from "lucide-react";
...
const CONTACT = [
  { icon: Mail, text: "aman.kumar@example.com" },
  { icon: Phone, text: "+91 98765 43210" },
  { icon: MapPin, text: "Bengaluru, India" },
];

/** Illustrative resume mockup for the hero — not a real screenshot. Clearly fictional
 * demo data. See docs/DESIGN-SYSTEM.md §Assets. */
  ...
  return (
    <div className="relative w-full max-w-md">
      <div className="overflow-hidden rounded-2xl border border-border-subtle bg-bg-card shadow-card">
        <div className="h-1.5 w-full bg-gradient-to-r from-brand-blue to-brand-cyan" />
        <div className="p-6">
          <p className="text-lg font-semibold text-text-primary">Aman Kumar</p>
          <p className="text-xs font-medium text-brand-blue">QA Automation Engineer</p>
          <div className="mt-2 flex flex-wrap gap-x-3 gap-y-1">
            {CONTACT.map((c) => (
              <span key={c.text} className="flex items-center gap-1 text-[10px] text-text-secondary">
          ...
          <p className="mt-1.5 text-[10px] font-medium text-text-primary">Automated Regression Suite</p>
          <p className="text-[9px] text-text-secondary">Selenium + Jenkins CI pipeline, 200+ test cases</p>
        </div>
      </div>
